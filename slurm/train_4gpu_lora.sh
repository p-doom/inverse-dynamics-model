#!/usr/bin/env bash
#SBATCH --job-name=idm-train-4gpu-lora
#SBATCH --nodes=1
...
torchrun \
    ...
    --val-steps 20 \
    --log-every 10 \
    --out-dir /fast/project/HFMI_SynergyUnit/p-doom/idm/train/runs/train_4gpu_lora_${SLURM_JOB_ID} \
    --wandb-project idm \
    --wandb-run-name "train-4gpu-lora-${SLURM_JOB_ID}" \
    --wandb-mode online \
    --seed 42
